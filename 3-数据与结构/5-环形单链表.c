...
void list_delete(List_t*list,ElemType_t delData)
{
    ...
    {
        //1 先找到尾节点
        Node_t *tail = list->head->next;
        while (1)
        {
            /* code */
            tail = tail->next;

            if(tail->next == list->head->next )
                break;
        }
        
        //2 尾节点的next成员存储删除节点的下一个节点地址
        tail->next = p->next;
        //3 头节点的next成员 存储 删除节点下一个节点地址
...
int main()
{
    ...
    list_delete(listA,60);
    list_show(listA);

    // list_insert_head(listA,80);
    // list_insert_head(listA,90);
    // list_show(listA);
    //链表B
