...
//如果查到了，返回查到的那个结构体指针
//否则返回NULL
Node_t* list_find(List_t*list,int id)
{
    //1、如果链表是空的，直接返回NULL
    if(isEmpty(list))
        return NULL;
    ...
    {
        /* code */
        if(p->data.id == id)//如果比较相同，则返回那个节点地址
            return p;
...
bool list_update(List_t*list,ElemType_t srcData,ElemType_t updateData)
{
    ...
    {
        /* code */
        if(p->data.id == srcData.id)//如果找到了，则直接更新
        {
            p->data = updateData;
            return true;
        }
...
int main()
{
    ...
    //链表A
    ElemType_t s1 = {2022100,22};
    list_insert_tail(listA,s1);

    ElemType_t s2 = {2022100,22};
    list_insert_tail(listA,s2);

    ElemType_t s3 = {2022100,22};
    list_insert_tail(listA,s3);
