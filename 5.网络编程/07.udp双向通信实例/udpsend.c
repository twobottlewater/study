...
void *myrecvmsg(void *arg)
{
	...
	while(1)
	{
		...
		ret=recvfrom(udpsock,rbuf,100,0,(struct sockaddr *)&addr,&size); //表示接收信息，同时把那个发送信息过来的主机ip和端口号存储
...
int main()
{
	...
	otheraddr.sin_port=htons(10086);  //对方的端口号
